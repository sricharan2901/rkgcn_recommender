...
def train(model, data, args):
    ...
    ripple_long = data["ripple_long"]
    ripple_short = data["ripple_short"]
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for start in range(0, n_train, args.batch_size):
            ...
            batch = get_batch_data(
                batch_idx, train_data, ripple_long, ripple_short, args.n_hop
            )

            # Prepare inputs
            inputs = {
                "items": tf.constant(batch["items"], dtype=tf.int32),
                "memories_h_long": [tf.constant(h, dtype=tf.int32) for h in batch["memories_h_long"]],
                "memories_r_long": [tf.constant(r, dtype=tf.int32) for r in batch["memories_r_long"]],
                "memories_t_long": [tf.constant(t, dtype=tf.int32) for t in batch["memories_t_long"]],
                "memories_time_long": [tf.constant(t, dtype=tf.float32) for t in batch["memories_time_long"]],
                "memories_h_short": [tf.constant(h, dtype=tf.int32) for h in batch["memories_h_short"]],
                "memories_r_short": [tf.constant(r, dtype=tf.int32) for r in batch["memories_r_short"]],
                "memories_t_short": [tf.constant(t, dtype=tf.int32) for t in batch["memories_t_short"]],
                "memories_time_short": [tf.constant(t, dtype=tf.float32) for t in batch["memories_time_short"]],
        ...
        train_auc, train_acc = evaluate(
            model, eval_train, ripple_long, ripple_short,
