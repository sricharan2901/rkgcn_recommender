...
class RKGCN(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        Args:
          inputs: dict with keys:
            ...
            - memories_h_long: list of n_hop arrays, each (batch_size, n_memory)
            - memories_r_long: list of n_hop arrays, each (batch_size, n_memory)
            - memories_t_long: list of n_hop arrays, each (batch_size, n_memory)
            - memories_time_long: list of n_hop arrays, each (batch_size, n_memory)
            - memories_h_short: list of n_hop arrays, each (batch_size, n_memory)
            - memories_r_short: list of n_hop arrays, each (batch_size, n_memory)
            - memories_t_short: list of n_hop arrays, each (batch_size, n_memory)
            - memories_time_short: list of n_hop arrays, each (batch_size, n_memory)
        ...
        memories_h_long = inputs["memories_h_long"]            
        memories_r_long = inputs["memories_r_long"]            
        memories_t_long = inputs["memories_t_long"]            
        memories_time_long = inputs["memories_time_long"]
        
        memories_h_short = inputs["memories_h_short"]            
        memories_r_short = inputs["memories_r_short"]            
        memories_t_short = inputs["memories_t_short"]            
        memories_time_short = inputs["memories_time_short"]
        ...
        # =============================================
        # Module 1: User Preference Aggregation (Dual Pipeline)
        # =============================================
        user_emb_long, kge_loss_long = self._user_preference_aggregation(
            item_embeddings, memories_h_long, memories_r_long, memories_t_long, memories_time_long
        )
        
        user_emb_short, kge_loss_short = self._user_preference_aggregation(
            item_embeddings, memories_h_short, memories_r_short, memories_t_short, memories_time_short
        )
        
        kge_loss = kge_loss_long + kge_loss_short
    ...
    def _user_preference_aggregation(self, item_embeddings, memories_h, memories_r, memories_t, memories_time):
        """
        ...
        Includes Time-Sensitive penalty logic.
        """
        o_list = []
        kge_loss = 0.0
        ...
        for hop in range(self.n_hop):
            ...
            # --- TS-RKGCN: Time Decay Penalty ---
            time_delta = memories_time[hop] # (B, M)
            decay_penalty = tf.nn.relu(self.time_decay_lambda) * time_delta
            att_scores = att_scores - decay_penalty
