...
def preprocess_movie(data_dir):
    ...
    # Positive: rating >= 4
    positive = ratings[ratings["rating"] >= 4][["user_id", "movie_id", "timestamp"]].copy()
    positive["label"] = 1
    ...
    user_ts = positive.groupby("user_id")["timestamp"].apply(list).to_dict()
    ...
    for uid, pos_items in user_pos.items():
        ...
        ts_list = user_ts.get(uid, [0])
        for iid in sampled:
            neg_ts = np.random.choice(ts_list)
            neg_rows.append({"user_id": uid, "item_id": iid, "label": 0, "timestamp": neg_ts})
...
def preprocess_book(data_dir):
    ...
    for uid, pos_items in user_pos.items():
        ...
        for iid in sampled:
            neg_rows.append({"user_id": uid, "item_id": iid, "label": 0, "timestamp": 0})
...
def reindex_and_save(ratings_final, kg_triples, item2entity, data_dir):
    ...
    ratings_final[["user_id", "item_id", "label", "timestamp"]].to_csv(
